...
Let's say you've got a few Docker Containers that are instantiated from Images that are pulled from hub.docker.io, or even a private registry. 
...
# Solution
One approach we can take (assuming using Docker Images from Docker Hub is acceptable), is to run CIS Benchmarks and DevSec Profiles against the image using `In-Spec` to ensure compliance

**NOTE:** for this post, I run the commands in a terminal but you can easily build your pipeline to do this.
